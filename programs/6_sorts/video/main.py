    ...
    def selection_sort(self) -> None:
        ...
        for spot in range(len(self.bars)):
            ...
            for looker in range(spot, len(self.bars)):
                self.bars[looker].color(Color.HIGHLIGHT)
                self.update_bars()
                if self.bars[looker] < self.bars[best]:
                    self.bars[best].color(Color.UNSORTED)
                    self.bars[looker].color(Color.BEST)
                    best = looker
                else:
                    self.bars[looker].color(Color.UNSORTED)
            self.swap_bars(spot, best)
            self.bars[spot].color(Color.SORTED)
            self.update_bars()
        self.update_bars()
